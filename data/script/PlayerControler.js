...
PlayerControl.prototype.Act = function() {
	if(this.act) {
		switch(telis.current_map) {
			case "Town" : 
				// 1125   1200   870  980
				if(this.inPotal(1180, 1230, 990, 1020)) { // Town 첫번째 집
				...
				if(this.inPotal(1540, 1660, 1960, 2030)) { // Town 맨 아래 포탈
			...
			break;
			case "Town_House1" : 
				if(this.inPotal(1150, 1200, 950, 970)) { // Town 첫번째 집
...
PlayerControl.prototype.inPotal = function(mx, _mx, my, _my) {
	if( (this.x + this.width / 2) > mx && (this.x + this.width / 2) < _mx && (this.y + this.height / 2) > my && (this.y + this.height / 2) < _my) { // Town 첫번째 집
		return true;
	}
	else {
		return false;
